...
router.get('/chat', forwardAuthenticated, (req, res) =>{
  MongoClient.connect('mongodb+srv://rithesh:rithesh@sandbox.0grtd.mongodb.net', function (err, client) {
    ...
    var room = {"code":req.query.room,"admin":req.query.username}
    if(req.query.username=="admin"){
      db.collection('rooms').insertOne(room, function(err, res) {
        if (err) throw err;
          console.log(res);
        db.close();
      });
      ...
      if (err) throw err;
      var db = client.db('studybuddy');
      db.collection('rooms').findOne({"code":req.query.room}, function(err,result){
      if (err) throw err;
        console.log(result);
        if(result==null){
...
router.get('/adminlogi', forwardAuthenticated, (req, res) => res.render('adminlogin'));
...
router.post ('/createroom', (req,res)=>{
    ...
    if (err) throw err;
    var db = client.db('studybuddy');
    var room = {"code":req.body['room'],"admin":req.body['username']}
    db.collection('rooms').insertOne(room, function(err, res) {
      if (err) throw err;
      console.log(res);
      db.close();
...
router.post('/admin', (req, res, next) => {
  ...
  var password = req.body["password"]
      ...
      console.log(result)
      db.close()
      if (result == 1) res.redirect('/users/adminlogi')
